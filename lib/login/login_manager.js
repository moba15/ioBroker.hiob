...
class LoginManager {
	...
	async onLoginRequest(client, deviceName, deviceID, key, user, password)  {
		...
		if(approved) {
			if(approved.val) {
				// @ts-ignore
				if(!password && needPWD && needPWD.val) {
					this.adapter.log.debug("Login approved for client (no pwd): " + client.toString() + " (" + deviceName + ") " + needPWD.val.toString());
					this.loginApprove(client);
				} else if(keyState.val == key) {
					if(!password) {
						password = "";
					}
