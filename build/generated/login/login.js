...
const _CompatibilityResponse = class _CompatibilityResponse extends pb_1.Message {
  constructor(data) {
    ...
    if (!Array.isArray(data) && typeof data == "object") {
      if ("compatible" in data && data.compatible != void 0) {
  ...
  static fromObject(data) {
    ...
    if (data.compatible != null) {
      message.compatible = data.compatible;
    }
    return message;
  }
  toObject() {
  ...
  serialize(w) {
    ...
    if (this.compatible != false)
      writer.writeBool(1, this.compatible);
    if (!w)
      return writer.getResultBuffer();
  }
  static deserialize(bytes) {
    ...
    while (reader.nextField()) {
      ...
      switch (reader.getFieldNumber()) {
        case 1:
          message.compatible = reader.readBool();
          break;
        default:
          reader.skipField();
      }
